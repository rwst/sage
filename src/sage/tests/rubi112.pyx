...
from sage.functions.all import (sqrt, log, sin, cos, arcsin, arccos, arctan, arctanh,
...
def test_7():
    test = [
        ...
        [(a + b*x)**m*(c + d*x)**n, x, ZZ(2), (a + b*x)**(ZZ(1) + m)*(c + d*x)**n*hypergeometric([ZZ(1) + m, - n], [ZZ(2) + m], - d*(a + b*x)/(b*c - a*d))/(b*(ZZ(1) + m)*(b*(c + d*x)/(b*c - a*d))**n)],
    ]
...
def test_numerical():
    test = [
        [(a + b*x)**(ZZ(3)/ZZ(2))*(c + d*x)**(ZZ(1)/ZZ(3)), ],
        [(a + b*x)**(ZZ(1)/ZZ(2))*(c + d*x)**(ZZ(1)/ZZ(4)), ],
        [(a + b*x)**(ZZ(3)/ZZ(2))/(c + d*x)**(ZZ(1)/ZZ(3)), ],
        [(a + b*x)**(ZZ(5)/ZZ(2))/(c + d*x)**(ZZ(1)/ZZ(6)), ],
        [(a + b*x)**(ZZ(5)/ZZ(2))/(c + d*x)**(ZZ(1)/ZZ(4)), ],
        [(a + b*x)**(ZZ(3)/ZZ(2))/(c + d*x)**(ZZ(1)/ZZ(5)), ],
    ]
